# NE302 OTA Full Bundle (One-Click Upgrade) Design

> English translation (the original Chinese `Docs/NE301_OTA_Bundle_Design.md`
> lives in the NE301 repository; this English copy is authoritative here and
...
> 0x24; `NE302 = 0x3020` (`NE301 = 0x3010`), 0 = unstamped legacy packages, allowed through).
> The device compares it against the compile-time `OTA_DEVICE_MODEL` at both
> precheck and upload — cross-model firmware (e.g. an NE301 package on an
> NE302) is hard-rejected. The single source of truth is the root Makefile's
...
- output: `build/ne302_Full_v<APP_VERSION>_bundle.bin`;
...
pkg-bundle: pkg
	@python $(PKG_SCRIPT_DIR)/ota_bundle_packer.py $(BUILD_DIR) \
	    -o $(BUILD_DIR)/ne302_Full_v$(APP_VERSION_STR)_bundle.bin -m $(DEVICE_MODEL)
```
