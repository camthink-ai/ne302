#!/usr/bin/env python3
"""
OTA Bundle Packer — builds the NE302 one-click full upgrade package.

Bundle layout (see Custom/Core/System/ota_bundle.h and
Script/docs/NE302_OTA_Bundle_Design_EN.md):
...
Usage:
    python ota_bundle_packer.py <build_dir> -o out.bin [--mem-map path]
                                [--exclude fsbl,wifi] [--board-flash 64]
"""
...
FW_SPECS = [
    (0x04, "model", "AI_1",   "ne302_Model_v*_pkg.bin"),
    (0x03, "web",   "WEB",    "ne302_Web_v*_pkg.bin"),
    (0x08, "wifi",  "WIFI_FW", "ne302_Wifi_flash_v*_pkg.bin"),
    (0x02, "app",   "APP1",   "ne302_App_signed_v*_pkg.bin"),
    (0x01, "fsbl",  "FSBL",   "ne302_FSBL_signed_v*_pkg.bin"),
]
...
def build_bundle(build_dir, out_path, mem_map_path, exclude, board_flash, model_id=0x3020):
...
def main():
    parser = argparse.ArgumentParser(description="NE302 OTA bundle packer")
    ...
    parser.add_argument("--board-flash", type=int, default=64, choices=[64, 128],
                        help="BOARD_FLASH_SIZE variant used to parse mem_map.h (default 64, matches Makefile COMMON_DEFS)")
    parser.add_argument("-m", "--model", type=lambda s: int(s, 0), default=DEVICE_MODEL,
                        help="device model id stamped at 0x24 (hex; 0x3020 = NE302, 0 = unstamped)")
